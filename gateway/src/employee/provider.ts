 ...
 * unknown fields. A run that already started keeps the runtime recorded on it, so a resumed task never
 * silently changes engine mid-flight. Otherwise AGENT_RUNTIME is the deployment's engine for every owner
 * who was not deliberately given a different one (runtimeFrom:'owner'); the runtime stamped on a profile
 * when it was first seeded is only a fallback, or editing .env would never move an existing owner.
 * Anything that is not a known runtime resolves to the preserved default rather than indexing into nothing. */
export type Runtime='hermes'|'anthropic'|'claude';
export function selectRuntime(profile:Row|undefined,run:Row):Runtime{const r=run.runtime??(profile?.runtimeFrom==='owner'?profile.runtime:process.env.AGENT_RUNTIME??profile?.runtime);return r==='hermes'||r==='claude'?r:'anthropic';}
